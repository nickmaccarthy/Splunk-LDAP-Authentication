...
class logger(object):
    ...
    def get_logger(self, name='ldap_auth'):
        
        logger = logging.getLogger(name)
        logger.setLevel(logging.DEBUG)

        SPLUNK_HOME = os.environ.get('SPLUNK_HOME')
        #LOG_FILENAME = os.path.join(SPLUNK_HOME, 'var', 'log', 'splunk', 'ldap_auth.log')
        path = ['/', 'usr', 'local', 'apps', 'splunk', 'var', 'log', 'splunk', 'ldap_auth.log']
        LOG_FILENAME = "/".join(path) 
